...
class ClosedBezierCurve extends SVGPath {
    ...
    ClosedBezierCurve(final ObjectExpression<Point2D> center) {
        ...
        this.controlPoints = Stream.of(control1, control2, control3, control4)
                                   .map(Arrays::asList)
                                   .flatMap(Collection::stream)
                                   .collect(Collectors.toList());

        final Consumer<PolarPoint> addListenerConsumer = pt -> {
    ...
    private PolarPoint getPoint(final DoubleExpression magnitude,
                                final DoubleExpression phase,
        ...
        point.centerXProperty().bind(Bindings.createDoubleBinding(() -> this.center.get().getX(), this.center));
        point.centerYProperty().bind(Bindings.createDoubleBinding(() -> this.center.get().getY(), this.center));
    ...
    private void updatePath() {
        ...
        final List<PolarPoint> points = Arrays.asList(this.point1, this.point2, this.point3, this.point4);
        final int max = Math.min(points.size(), this.controlPoints.size() / 2) - 1;
        final String path = IntStream.rangeClosed(0, max).mapToObj(i -> {
    ...
    ControlPoint[] getControlPointsForNode(final int node) {
        return new ControlPoint[] {
            this.controlPoints.get(2 * node),
            this.controlPoints.get(2 * node + 1)
        };
    }
